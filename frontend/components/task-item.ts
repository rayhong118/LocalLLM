...
export class TaskItem extends LitElement {
    ...
    private _getTimezoneAbbreviation() {
        if (!TaskItem._tzAbbreviation) {
            TaskItem._tzAbbreviation = new Intl.DateTimeFormat('en-US', { timeZoneName: 'short' })
                .formatToParts(new Date())
                .find(p => p.type === 'timeZoneName')?.value || '';
        }
        return TaskItem._tzAbbreviation;
    }
    ...
    private _dispatchAction(eventName: string) {
        this.dispatchEvent(new CustomEvent(eventName, { 
            detail: { taskId: this.task.id, status: this.task.status },
            bubbles: true, 
            composed: true 
        }));
    }
    ...
    override render() {
        ...
        const hasOutput = this.task.outputs && this.task.outputs.length > 0 && this.task.status !== 'RUNNING';

        return html`
            <div class="task-item ${this.task.status}">
                <button class="delete-btn" title="Delete" @click=${(e: Event) => { e.stopPropagation(); this._dispatchAction('task-delete'); }}>&times;</button>
                
                <div class="task-header">
                    <div style="display: flex; gap: 0.75rem; align-items: center; flex: 1; min-width: 0;">
                        <span class="status-badge ${this.task.frequency === 'DAILY' ? 'DAILY' : this.task.status}">${this.task.frequency === 'DAILY' ? 'DAILY' : this.task.status}</span>
                        <span class="prompt-text">${this.task.prompt}</span>
                    </div>
                    <div class="task-actions">
                        ${this.task.frequency === 'DAILY' && this.task.status !== 'RUNNING' ? html`
                            <button class="action-btn retry-btn" @click=${(e: Event) => { e.stopPropagation(); this._dispatchAction('task-run-now'); }}>Run Now</button>
                        ` : ''}
                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span class="time">
                        ${this.task.frequency === 'DAILY' ? `Every day at ${String(this.task.hour_of_day).padStart(2, '0')}:00 ${this._getTimezoneAbbreviation()}` :
                this.task.status === 'RUNNING' ? `Started: ${this._toLocalDate(this.task.started_at).toLocaleTimeString()} ${this._getTimezoneAbbreviation()}` :
                    this.task.status === 'CANCELLED' ? `Cancelled at: ${this._toLocalDate(this.task.updated_at).toLocaleTimeString()} ${this._getTimezoneAbbreviation()}` :
                        `Last Run: ${this._toLocalDate(this.task.updated_at).toLocaleString()} ${this._getTimezoneAbbreviation()}`}
                    </span>
                    ${hasOutput ? html`
                        <button class="view-result-btn"
                              @click=${() => this._isExpanded = !this._isExpanded}>
                            ${this._isExpanded ? 'Hide Result ↑' : 'View Result ↓'}
                        </button>
                    ` : ''}
                </div>
